...
```js
Poyo.loadBitmapFont(path, grid_w, grid_h, rows, padding_x, padding_y, sequence)
```
...
`padding_x` is an offset applied to the bitmap font's `grid_width` property during rendering, so values greater than zero _increase_ the distance between glyphs, whereas values less than zero _decrease_ said distance. If no value is supplied, `0` is used. Likewise, `padding_y` is an offset applied to the bitmap font's `grid_height` property and is `0` by default. Tweak `padding_x` to adjust letter spacing, and tweak `padding_y` to adjust line height.
